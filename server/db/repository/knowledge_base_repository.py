...
@with_session
def update_video_path_in_db(session, kb_name, video_path):
    kb: KnowledgeBaseModel = session.query(KnowledgeBaseModel).filter_by(kb_name=kb_name).first()
    if kb:
        kb.video_path = video_path
        session.commit()  # 确保更改提交到数据库
        return True
...
def load_kb_from_db(session, kb_name):
    kb = session.query(KnowledgeBaseModel).filter_by(kb_name=kb_name).first()
    if kb:
        kb_name, vs_type, embed_model, video_path = kb.kb_name, kb.vs_type, kb.embed_model, kb.video_path
    else:
        kb_name, vs_type, embed_model, video_path = None, None, None, None
    return kb_name, vs_type, embed_model, video_path
