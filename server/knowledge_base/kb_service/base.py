...
from server.db.repository.knowledge_base_repository import (
    ...
    load_kb_from_db, get_kb_detail, update_video_path_in_db,
)
...
class KBService(ABC):
    ...
    所有细分 kb 服务的父类
    只提供所有 kb 都需要的服务：
    - 加载 embeddings模型：_load_embeddings()
        - 根据 模型名 和 device 去 embeddings模型cache 里找（不会重复创建多个）
    - 创建 kb
    - 清理 kb
    - 添加 doc
    ...
    ...
    def update_video_path_in_kb(self, video_path):
        # 首先更新数据库中的 video_path
        try:
            update_video_path_in_db(self.kb_name, video_path)
            self.video_path = video_path
        except Exception as e:
            return e

    def get_video_path_in_kb(self):
...
class KBServiceFactory:
    ...
    def get_service(kb_name: str,
        ...
        if SupportedVSType.FAISS == vector_store_type:
            ...
            return FaissKBService(kb_name, embed_model=embed_model, video_path=video_path)
        if SupportedVSType.PG == vector_store_type:
            from server.knowledge_base.kb_service.pg_kb_service import PGKBService
            return PGKBService(kb_name, embed_model=embed_model, video_path=video_path)
        elif SupportedVSType.MILVUS == vector_store_type:
            from server.knowledge_base.kb_service.milvus_kb_service import MilvusKBService
            return MilvusKBService(kb_name,
                                   embed_model=embed_model,
                                   video_path=video_path)  # other milvus parameters are set in model_config.kbs_config
    ...
    def get_service_by_name(kb_name: str
        ...
        # 从数据库拿到某个kb的信息
        _, vs_type, embed_model, video_path = load_kb_from_db(kb_name)
        if vs_type is None and os.path.isdir(get_kb_path(kb_name)):  # faiss knowledge base not in db
            vs_type = "faiss"
        # 根据数据库拿到的信息 返回kb实例
        return KBServiceFactory.get_service(kb_name, vs_type, embed_model, video_path)
